...
for ($i = 1; $i < count($data); $i++) {
    ...
    if (!empty($data[$i])) {
        // Tambahkan tag pembuka dan penutup untuk setiap entri XML
        $xml_string = '<Row>' . $data[$i] . '</Row>';
        
        // Load string XML menggunakan SimpleXML
        $xml = simplexml_load_string($xml_string);
        
        // Jika XML berhasil dimuat, tambahkan data ke array hasil
        if ($xml !== false) {
            $row_data = json_decode(json_encode($xml), true);
            $extracted_data[] = $row_data;
        } else {
            // Tampilkan pesan kesalahan jika XML tidak valid
            echo "Error parsing XML: " . $data[$i];
        }
    }
}
?>
